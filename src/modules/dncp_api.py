"""
    ...
    Additionally, the module contains a function to obtain a list of awarded suppliers for a given tender using the API V3
"""
import json
import requests
import pandas
...
def obtain_access_token_dncp_V3(requestToken):
    ...
    try:
        response = requests.post(accessTokenURL_V3, json=body, headers=head)
...
class licitaciones:
    def __init__(self, estado, link, nombreCSV, accessToken_V3):
        """
    ...
    def obtain_values(self, ID, status):
        """
        returns a dictionary with the values corresponding to an individual ID, using the V3 API
        ...
        if status == "ADJ":
            urlAPI = tenderAdjudicadosEndPint_V3 + ID
            adjudicados = self.__obtain_api_v3_adjudicados(urlAPI)
            if adjudicados != False:
                dicValues["Adjudicados"] = ', '.join(adjudicados)
            else:
                print("ERROR obteniendo proveedores por medio de la api V3.")
    ...
    def __obtain_api_v3_adjudicados(self, urlAPI):
        ...
        head = {
            "Authorization":"Bearer " + self.accessToken_V3
        }
